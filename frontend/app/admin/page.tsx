              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                {eventPhotos.map((photo) => (
                  <div key={photo.photo_id} className="relative group aspect-square rounded-lg overflow-hidden bg-slate-100">
                    <img 
                      src={`${apiUrl}${photo.file_path.replace(/([^/]+)$/, 'thumb_$1')}`} 
                      onError={(e) => {
                        // Fallback to original image if thumbnail is not yet generated by the background worker
                        e.currentTarget.src = `${apiUrl}${photo.file_path}`;
                      }}
                      alt="Event photo" 
